...
@router.get("/")
async def list_my_timezones(logged_user: Dict = Depends(JWTBearer())):
    return timezone_service.list_tz_by_user_id(logged_user["user_id"])


@router.get("/users/{user_id}")
async def list_user_timezones(user_id: int, logged_user: Dict = Depends(JWTBearer())):
    check_logged_is_admin(logged_user=logged_user)
    return timezone_service.list_tz_by_user_id(user_id)


@router.post("/")
async def create_my_timezone(tz: TimezoneCreate, logged_user: Dict = Depends(JWTBearer())):
...
@router.put("/")
async def update_my_timezone(tz: TimezoneUpdate, logged_user: Dict = Depends(JWTBearer())):
    owner_id = logged_user["user_id"] if tz.owner_id is None else tz.owner_id
    check_logged_own_or_is_admin(logged_user=logged_user, user_id=owner_id)
    try:
